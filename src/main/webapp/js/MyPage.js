...
//分页对象
function Page(){
    this.config = {elemId:'#page0',pageIndex:'1',total:'0',pageNum:'10',pageSize:'20'};//默认参数
    ...
    this.pageHtml = function(){
        ...
        if(pageIndex <= 0 || pageIndex == 1){
            pageHtml += '<li class="disabled"><a href="javascript:indexpage('+pageIndex+');" data-pageindex="'+ pageIndex +'">首页</a></li>' +
                '<li class="disabled"><a href="javascript:indexpage('+pageIndex+');" data-pageindex="'+ pageIndex +'">上一页</a></li>';
        }else{
            pageHtml += '<li><a href="javascript:indexpage(1);" data-pageindex="1">首页</a></li>' +
                '<li><a href="javascript:indexpage('+(pageIndex - 1)+');" data-pageindex="'+ (pageIndex - 1) +'">上一页</a></li>';
        }

        for(var i = k;i < (k + j);i++){
            if(pageTotal == (pageIndex + i - 1))break;
            if(i == 0){
                pageHtml += '<li class="active"><a href="javascript:indexpage('+pageIndex+');" data-pageindex="'+ pageIndex +'">'+ pageIndex +'</a></li>';
            }else{
                pageHtml += '<li><a href="javascript:indexpage('+(pageIndex+i)+');" data-pageindex="'+ (pageIndex + i) +'">'+ (pageIndex + i) +'</a></li>';
            }
        }

        if(pageTotal == 1 ||  pageTotal <= pageIndex){
            pageHtml += '<li class="disabled"><a href="javascript:indexpage('+pageTotal+');" data-pageindex="'+ pageTotal +'">下一页</a></li>' +
                '<li class="disabled"><a href="javascript:indexpage('+pageTotal+');" data-pageindex="'+ pageTotal +'">末页</a></li>';
        }else{
            pageHtml += '<li><a href="javascript:indexpage('+(pageIndex+1)+');" data-pageindex="'+ (pageIndex + 1) +'">下一页</a></li>' +
                '<li><a href="javascript:indexpage('+pageTotal+');" data-pageindex="'+ pageTotal +'">末页</a></li>';
        }
        pageHtml += '</ul>'
        $(elemId).html('');
